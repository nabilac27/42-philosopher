...
/*   Updated: 2025/08/23 07:46:25 by nchairun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/philo.h"

void assign_forks(t_philo *philo, t_fork *forks, int philo_pos, int num_philos)
{
    // Normal assignment: left = current, right = next
    philo->left_fork  = &forks[philo_pos];
    philo->right_fork = &forks[(philo_pos + 1) % num_philos];

    // Deadlock prevention: even philosophers pick right fork first
...
void init_philo_struct(t_table *table)
{
    ...
    while (i < table->num_philos)
    {
        ...
        // Assign forks using separate function
